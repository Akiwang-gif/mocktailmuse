...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildStaticSeoMetadata({
    title: pages.recipeDisclaimer.metadataTitle,
    description: pages.recipeDisclaimer.metadataDescription,
    path: "/recipe-disclaimer",
  });
}

export default async function RecipeDisclaimerPage() {
  const [{ pages }, identity] = await Promise.all([getResolvedLegalConfig(), getSiteIdentitySettings()]);
  const page = pages.recipeDisclaimer;
